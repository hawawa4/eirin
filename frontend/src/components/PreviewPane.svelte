<script lang="ts">
  ...
  let rawInfo: { width: number; height: number; channels: number; stats: fits.ChannelStats[] } | null = null
...
void main() {
  ...
  function computeUniforms(
    ...
    const presets = [
      { shadowsFactor: -1.25, targetBG: 0.10 }, // level 1 – gentle
      { shadowsFactor: -2.80, targetBG: 0.25 }, // level 2 – normal (Siril default)
      { shadowsFactor: -4.00, targetBG: 0.40 }, // level 3 – strong
    ]
    ...
    return stats.map(s => {
      ...
      if (!enabled) {
        ...
        return { shadows: Math.max(0, med - 2.80 * sig), midtone: 0.5, linear: true }
      }
  ...
  $effect(() => {
    ...
    Promise.allSettled([
      ...
      GeneratePreviewRaw(e.path),
    ]).then(([hdrResult, rawResult]) => {
      ...
      if (rawResult.status === 'fulfilled') {
        ...
        const snapSe = se; const snapSl = sl
